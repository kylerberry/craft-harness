...
After a soft inspection warning, such as a non-fatal tool denial or partial read, stop inspecting and finalize from the current evidence using one of the two shapes. Note the evidence gap and do not guess. Use the blocked shape only when required input is absent; otherwise return the report shape.
